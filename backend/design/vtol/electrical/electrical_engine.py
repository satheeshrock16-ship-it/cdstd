...
class ElectricalEngine:
    ...
    def design_electrical_system(self, requirements: ElectricalRequirements) -> ElectricalResult:
        ...
        # 2. Sizing battery capacity & configuration S/P
        mtow = mission_res.mission_analysis.estimated_mtow_kg
        energy_demand_wh = mission_res.mission_profile.total_energy_demand_kwh * 1000.0
        f_reserve = strategy.default_reserve_factor
        req_energy_wh = energy_demand_wh * f_reserve
        ...
        # 3. Sizing Power Budget
        lift_hover_w = lift_res.power_analysis.hover_total_power_kw * 1000.0
        fwd_cruise_w = forward_res.power_analysis.required_cruise_power_w

        slots = [
            PowerBudgetSlot("Vertical Lift Motors", round(lift_hover_w, 1), 0.0, round(lift_hover_w * 0.75, 1)),
        ...
        total_hover_w = sum(s.hover_power_w for s in slots)
        total_cruise_w = sum(s.cruise_power_w for s in slots)
        total_trans_w = sum(s.transition_power_w for s in slots)
        ...
        # 4. Sizing Power Distribution Buses
        hover_current = lift_res.power_analysis.hover_total_current_a
        cruise_current = forward_res.power_analysis.current_draw_a
        ...
        # 10. Consolidated analysis indices
        hover_energy = total_hover_w * (mission_res.hover_requirements.hover_duration_min / 60.0)
        cruise_energy = total_cruise_w * (mission_res.cruise_requirements.cruise_endurance_min / 60.0)
        trans_energy = total_trans_w * (mission_res.transition_requirements.transition_duration_s / 3600.0)
