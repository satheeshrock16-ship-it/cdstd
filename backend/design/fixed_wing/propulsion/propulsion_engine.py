...
class PropulsionEngine:
    ...
    def process_propulsion_design(
        ...
        if wing_geom and getattr(wing_geom, "wing_loading_kg_m2", None) and getattr(wing_geom, "area_m2", None):
            mtow = wing_geom.wing_loading_kg_m2 * wing_geom.area_m2
        elif m_profile.maximum_takeoff_weight_limit_kg:
            mtow = m_profile.maximum_takeoff_weight_limit_kg
            ...
            mtow = m_profile.payload_kg / 0.25
        weight_n = mtow * g
        ...
        constraints = PropulsionConstraints(
            ...
            max_prop_diameter_m=max(0.28, requirements.fuselage_result.fuselage_geometry.height_m * 2.0),
        ...
        # Sized Cruise Power: P_cruise = (Thrust * V_flight) / eta
        cruise_power_w = (cruise_drag_n * v_cruise_m_s) / eta_total

        # Sized Climb Power: assume climb speed is 1.3 * V_stall, ROC = 3.5 m/s
        ...
        # Maximum takeoff/shaft power target (accounting for climb, cruise, and static takeoff thrust)
        ...
        if prop_type == PropulsionType.ELECTRIC:
            ...
            for motor in motors:
                for propeller in props:
                    ...
                    # static thrust and power margin calculations
                    prop_area_val = (math.pi / 4.0) * (propeller.diameter_m ** 2)
                    shaft_max_power = motor.max_power_w * profile.default_motor_efficiency
                    static_thrust_n = (m_profile.air_density_kg_m3 * prop_area_val * (shaft_max_power ** 2)) ** (1.0 / 3.0)
                    t_w_ratio = static_thrust_n / weight_n
                    ...
                    climb_excess_power_val = (motor.max_power_w * profile.default_motor_efficiency * profile.default_prop_efficiency) - (cruise_drag_n * climb_speed_m_s)
                    roc_val = climb_excess_power_val / weight_n
                    if roc_val < 0.5:
                        continue
                        
                    # Deterministic ranking metrics
                    thrust_margin = static_thrust_n - takeoff_thrust_n
        ...
        # 8. Sizing analyses
        # Est static thrust: T = (rho * Area * (Power_shaft)^2)^(1/3)
        prop_area = (math.pi / 4.0) * (prop.diameter_m ** 2)
        shaft_max_power = max_component_power * profile.default_motor_efficiency
        static_thrust_n = (m_profile.air_density_kg_m3 * prop_area * (shaft_max_power ** 2)) ** (1.0 / 3.0)

        t_anal = ThrustAnalysis(
            required_cruise_thrust_n=round(cruise_drag_n, 2),
            required_takeoff_thrust_n=round(takeoff_thrust_n, 2),
            estimated_static_thrust_n=round(static_thrust_n, 2),
            thrust_to_weight_ratio=round(static_thrust_n / weight_n, 2),
            power_loading_w_kg=round(max_component_power / mtow, 2),
        )

        p_anal = PowerAnalysis(
            required_cruise_power_w=round(cruise_power_w, 1),
            required_climb_power_w=round(climb_power_w, 1),
            maximum_power_w=round(max_component_power, 1),
            current_draw_cruise_a=round(cruise_power_w / voltage_v, 2) if voltage_v > 0.0 else 0.0,
            metadata={"voltage_v": voltage_v, "motor_weight_g": weight_g},
        )

        # Wh/km = Wh draw per hour / speed_kmh = Watts / speed_kmh
        energy_rate = cruise_power_w / m_profile.cruise_speed_kmh
        ...
        climb_excess_power = (max_component_power * profile.default_motor_efficiency * profile.default_prop_efficiency) - (cruise_drag_n * climb_speed_m_s)
        ...
        t_to_w_actual = static_thrust_n / weight_n
        ...
        to_accel = to_vel / max(0.1, (static_thrust_n - cruise_drag_n) / mtow)

        to_anal = TakeoffAnalysis(
            takeoff_distance_m=round(to_dist, 2),
            takeoff_velocity_m_s=round(to_vel, 2),
            acceleration_force_n=round(static_thrust_n - cruise_drag_n, 2),
        ...
        engineering_notes = [
            ...
            f"Selected power unit: {selected_component_name} (max power: {max_component_power:.1f} W).",
            f"Sized propeller: {prop.name} (diameter: {prop.diameter_in:.1f} in, pitch: {prop.pitch_in:.1f} in).",
